...
const paintPersons = (randomPerson) => {
    const board$$ = document.querySelector('[data-function="boardGame"]');
    persons.forEach(per => {
        const img$$ = document.createElement('img');
        img$$.classList.add('person-img')
        img$$.setAttribute('src', `${per.img}`);
        img$$.addEventListener('click', () => checkPerson(per, randomPerson));
        board$$.appendChild(img$$);

    })
}

const paintQuestions = (randomPerson) => {
    const questions$$ = document.querySelector('[data-function="questions"]');
    questionsType.forEach(ques => {
        const ques$$ = document.createElement('div');
        ques$$.innerHTML = `
            <h2>${ques.title}</h2>
            ${ques.questions.map(q => `<button>${q}</button>`).join('')}
        `
        questions$$.appendChild(ques$$);
        ques$$.querySelectorAll('button').forEach(btn => {
            btn.addEventListener('click', (e) => checkFeature(e.target, ques.key, randomPerson, btn.textContent));
        })
    })
}

const checkFeature = (btn, feature, randomPerson, selected) => {
    const personsImg$$ = document.querySelectorAll('.person-img');
    const cluesCounter$$ = document.querySelector('[data-function="clueCount"]');
    let cluesCount = Number(cluesCounter$$.textContent);

    if(randomPerson[feature] === selected) {
        persons.map((per, i) => {
            if(per[feature] !== selected) {
                personsImg$$[i].classList.add('disabled');
            }         
        })
    } else {
        persons.map((per, i) => {
    ...
    cluesCount++
    cluesCounter$$.textContent = cluesCount;
}

const checkPerson = (clicked, randomPerson) => {
    const result = JSON.stringify(clicked) === JSON.stringify(randomPerson);
    if(result) {
        const clues = document.querySelector('[data-function="clueCount"]').textContent;
        alert(`you won using ${clues} clues!`);
    } else {
        alert('you lost...');
    }
...
const resetGame = () => {
    document.querySelector('.b-board-game').innerHTML = '';
    document.querySelector('.b-questions').innerHTML = '';
    document.querySelector('[data-function="clueCount"]').textContent = '0';
}
const init = () => {
    resetGame();
    const randomPosition = Math.floor(Math.random() * persons.length);
    const randomPerson = persons[randomPosition];

    paintPersons(randomPerson);
    paintQuestions(randomPerson);
}
